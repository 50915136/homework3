...
private:
    Term* head;//用來作為多項式項的鏈表的頭部。head 節點本身並不存儲實際的多項式項目，但它用來標記鏈表的開始並提供一個循環結構。
    void insertTerm(float coef, int exp);//根據係數和指數在鏈表中找到正確的位置來插入新的 Term 對象。這個方法會處理合併相同指數的項，並在需要時刪除係數為零的項。
    void clear();//清空多項式中所有項目的私有方法。它會遍歷鏈表，刪除所有 Term 對象，然後重設 head 節點的鏈接，使其指向自身。
};

Polynomial::Polynomial() 
{
    head = new Term;//這個 Term 對象作為鏈表的頭部節點，用於標記鏈表的起始位置。
    head->link = head;//形成一個循環鏈表。這樣設置可以使得當鏈表中沒有其他節點時，head->link 仍然指向自己，簡化了邊界條件處理。
}
...
Polynomial::Polynomial(const Polynomial& poly) 
{
    head = new Term;
    head->link = head;
    Term* current = poly.head->link;//設置 current 指針為傳入 poly 對象鏈表中的第一個實際節點。
    while (current != poly.head)//遍歷 poly 對象中的所有 Term 節點（直到返回到 head）。
    {
        insertTerm(current->coef, current->exp);//將每個 Term 的係數和指數插入到當前 Polynomial 對象中。
...
Polynomial::~Polynomial() 
{
    clear();//刪除多項式中所有的 Term 對象。這個方法會遍歷鏈表並刪除每個節點，確保沒有內存泄漏。
    delete head;//刪除 head 指針所指向的節點。這裡的 head 節點在構造函數中是用 new 分配的，因此需要顯式地刪除來釋放內存。
}

void Polynomial::clear() //刪除多項式中的每一個 Term 對象，以便釋放佔用的內存並確保鏈表的狀態被重置為空
{
    Term* current = head->link;//第一個實際 Term 節點
    while (current != head) //遍歷鏈表中的所有 Term 節點。循環的條件是 current 不等於 head，這樣可以確保循環繼續直到回到 head 節點為止
    {
        Term* temp = current;//在刪除當前節點後仍然能夠保留對該節點的指針。這樣可以確保在刪除節點之前能夠正確地移動到下一個節點。
        current = current->link;//更新 current 指針，將其設置為當前節點的 link 成員
        delete temp;//刪除當前節點以釋放內存
    }
    head->link = head;//重新建立一個空的循環鏈表結構。這樣，鏈表的狀態被重置為空，並且 head 節點指向自身。
}
//Polynomial::表示insertTerm 函數是 Polynomial 類的一部分，而不是全局函數或其他類的函數。
void Polynomial::insertTerm(float coef, int exp) //用於在多項式中插入一個新的項，或者更新已有項的係數。如果插入或更新後的係數為零，則該項會被刪除。
{
    ...
    if (current != head && current->exp == exp) //找到的節點 (current) 的指數等於新項的指數，則更新該節點的係數。
    {
        current->coef += coef;
        if (current->coef == 0) //更新後的係數為零，則刪除該節點並調整鏈表中的鏈接，以移除該項。
        {
...
const Polynomial& Polynomial::operator=(const Polynomial& poly) //將一個 Polynomial 對象的內容賦值給另一個 Polynomial 對象。
{
